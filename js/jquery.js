(function(window, undefined) {
	...
	boolHook = {
		get: function(elem, name) {
			...
			prop = jQuery.prop(elem, name),
				...
				getSetInput && getSetAttribute ? attr != null : ruseDefault.test(name) ? elem[jQuery.camelCase("default-" + name)] : !! attr :
	...
	jQuery.fn.extend({
		...
		is: function(selector) {
			return !!selector && (
			typeof selector === "string" ? rneedsContext.test(selector) ? jQuery(selector, this.context).index(this[0]) >= 0 : jQuery.filter(selector, this).length > 0 : this.filter(selector).length > 0);
		},
	...
	jQuery.extend({
		...
		ajax: function(url, options) {
			...
			var
			parts, i, cacheURL, responseHeadersString, timeoutTimer,

			fireGlobals,

			transport, responseHeaders, s = jQuery.ajaxSetup({}, options),
		...
		jQuery.each({
			...
			jQuery.fn[funcName] = function(margin, value) {
				...
				return jQuery.access(this, function(elem, type, value) {
					...
					return value === undefined ? jQuery.css(elem, type, extra) :
